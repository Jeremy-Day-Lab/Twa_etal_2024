...
#### subset for acute data set
testing.NAc@active.ident <- factor(testing.NAc$Dataset)
testing.NAc <- subset(testing.NAc, idents = "Acute")
...
g <- ggroc(data = list(`Chr Y` = chrY_roc,
  ...
  labs(color = "Model") +
  theme_bw()
ggsave(filename = "/scratch/gtwa/Day/sex_prediction_model/output/NAc_evaluation/ROC_curves_acute.pdf",
...
write.csv(auc.table, "/scratch/gtwa/Day/sex_prediction_model/output/NAc_evaluation/AUCs_table_acute.csv")
...
write.csv(acc.table, "/scratch/gtwa/Day/sex_prediction_model/output/NAc_evaluation/Accuracy_table_acute.csv")
...
ggsave(plot = confusion.plot,
       file = "/scratch/gtwa/Day/sex_prediction_model/output/NAc_evaluation/confusionplot_acute.pdf",
       height = 7,
       width = 10,
       device = "pdf")
