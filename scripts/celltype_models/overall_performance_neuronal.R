...
## data
### models
svm_model <- readRDS("/scratch/gtwa/Day/sex_prediction_model/data/celltype_models/models/svm_rad_model_neuronal.RDS")
rf_model <- readRDS("/scratch/gtwa/Day/sex_prediction_model/data/celltype_models/models/rf_model_neuronal.RDS")
mlp_model <- readRDS("/scratch/gtwa/Day/sex_prediction_model/data/celltype_models/models/mlpwdml_model_neuronal.RDS")
lr_model <- readRDS("/scratch/gtwa/Day/sex_prediction_model/data/celltype_models/models/lr_model_neuronal.RDS")
chrY_model <- read.csv("/scratch/gtwa/Day/sex_prediction_model/output/celltype_models/Yclass.predictions.neuronal.csv", row.names = 1)
Xist_model <- read.csv("/scratch/gtwa/Day/sex_prediction_model/output/celltype_models/Xist.class.predictions.neuronal.csv", row.names = 1)

### testing data
testing <- read.csv("/scratch/gtwa/Day/sex_prediction_model/data/celltype_models/test_count_data_neuronal.csv", row.names = 1)
...
g <- ggroc(data = list(`Chr Y` = chrY_roc,
  ...
  labs(color = "Model") +
  theme_bw()
ggsave(filename = "/scratch/gtwa/Day/sex_prediction_model/output/celltype_models/eval/ROC_curves_neuronal.pdf",
...
write.csv(auc.table, "/scratch/gtwa/Day/sex_prediction_model/output/celltype_models/eval/AUCs_table_neuronal.csv")
...
write.csv(acc.table, "/scratch/gtwa/Day/sex_prediction_model/output/celltype_models/eval/Accuracy_table_neuronal.csv")
...
ggsave(plot = confusion.plot,
       file = "/scratch/gtwa/Day/sex_prediction_model/output/celltype_models/eval/confusionplot_neuronal.pdf",
       height = 7,
       width = 10,
       device = "pdf")
